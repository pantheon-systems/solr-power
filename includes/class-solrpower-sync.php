...
class SolrPower_Sync {
	...
	/**
	 * Performs index if post status changes to publish.
	 *
	 * @param int $post_id
	 */
	...
	/**
	 * Deletes post from Solr if deleted in WordPress.
	 *
	 * @param int $post_id
	 */
	...
	/**
	 * Copies config settings to newly activated blog site and performs an index.
	 *
	 * @param int $blogid
	 */
	...
	/**
	 * Deletes posts in Solr if blog site is deleted.
	 *
	 * @param int $blogid
	 */
	...
	/**
	 * Deletes post from Solr during post status change.
	 *
	 * @param int $post_id
	 * @param WP_Post|null $post_info
	 */
	...
	/**
	 * Builds a Solr document for indexing.
	 *
	...
	function build_document(
		...
		if ( $post_info ) {
			...
			if ( is_multisite() && in_array( substr( site_url(), 7 ) . $post_info->ID, (array) $exclude_ids, true ) ) {
				return null;
			} elseif ( ! is_multisite() && in_array( $post_info->ID, (array) $exclude_ids, true ) ) {
				return null;
			}
	...
	/**
	 * Performs indexing action based upon created documents.
	 *
	...
	/**
	 * Copies main plugin options to other blogs.
	 */
