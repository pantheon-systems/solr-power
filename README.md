...
relevancy ratings accordingly before that data can appear in search results. By default, SolrPower does this
when it sends every post. It may be necessary on occasion to disable this behavior (e.g. when importing a lot
of posts via CSV). To do this, you need add the following code to your index.php in the root of your site install:

```php
define( 'SOLRPOWER_DISABLE_AUTOCOMMIT', true );
```

When this variable is defined, SolrPower will not commit the index until the cron runs. By default, the cron runs
on the pantheon platform every hour.

To force-commit data when this variable is defined outside of a normal cron run, from the command line, you can run the
command below or simply force a cron-run.
