...
if ( ! isset( $results['results'] ) || null === $results['results'] ) {
		...
		foreach ( $results['results'] as $result ) {
			...
			foreach ( explode( '...', $result['teaser'] ) as $this_result ) {
				if ( ! empty( $this_result ) ) {
					echo '...' . $this_result . '...<br /><br />'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped,WordPressDotOrg.sniffs.OutputEscaping.UnescapedOutputParameter
				}
			}
	...
	if ( $results['facets']['selected'] ) {
		foreach ( $results['facets']['selected'] as $selectedfacet ) {
			printf( '<li><span></span><a href="%s">%s<b>x</b></a></li>', $selectedfacet['removelink'], $selectedfacet['name'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
	?>
				</ol>
